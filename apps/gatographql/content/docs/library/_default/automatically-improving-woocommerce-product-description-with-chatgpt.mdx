...
To do this, first [create a new persisted query](/guides/use/creating-a-persisted-query) with the GraphQL query above, and title `"Improve Product Content With ChatGPT"` (this will assign it slug `improve-product-content-with-chatgpt`).

Then, anywhere in your application (eg: in your `functions.php` file, a plugin, or a code snippet), add the following PHP code:
