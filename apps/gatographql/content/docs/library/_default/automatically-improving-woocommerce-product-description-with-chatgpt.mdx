...
For that, go to the Settings page, click on the "Schema Elements Configuration > Custom Posts" tab, and select `product` from the list of queryable CPTs (if it's not selected already).
...
In addition, you can override the default value for variables `$model` (`"gpt-3.5-turbo-instruct"`) (check the [list of OpenAI models](https://platform.openai.com/docs/models)) and `$temperature` (`0.7`), and be aware to provide `$maxTokens` with a suitable number for the length of the content (`50` by default).
...
Then, anywhere in your application (eg: in your `functions.php` file, a plugin, or a code snippet), add the following PHP code, which executes the query on hook `save_post_product`:
...
add_action(
  'save_post_product',
  function (int $productId, WP_Post $post, bool $isUpdate): void {
    // Only execute when it's a new product
    if ($isUpdate) {
      return;
    }
    ...
    // (You can provide it via a constant `OPENAI_API_KEY` defined in `wp-config.php`:
    // $openAI_api_key = constant('OPENAI_API_KEY');

    // Customize any of the other variables
    $model = 'gpt-4o';
    $maxTokens = 5000;
