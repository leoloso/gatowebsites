...
Then, anywhere in your application (eg: in your `functions.php` file, a plugin, or a code snippet), add the following PHP code, which executes the query on hook ``:
...
add_action(
  ...
  function (int $productId, WP_Post $post, bool $isUpdate): void {
    // Only execute when it's a new entry
    if ($isUpdate) {
      return;
    }
