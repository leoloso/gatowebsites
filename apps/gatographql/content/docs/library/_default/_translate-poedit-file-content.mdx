...
This GraphQL query translates the empty strings from a Poedit file (by executing a single call to the Google Translate API) that is accessible under some URL, and adds those translations in the corresponding entry of a new translation Poedit file, for some language.

It then uploads the translation file to [Filestack](https://www.filestack.com), under the application for the given API key.
