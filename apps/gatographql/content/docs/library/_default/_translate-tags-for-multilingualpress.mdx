---
title: Translate tags for MultilingualPress
description: Translate a tag to all languages as defined in MultilingualPress, and store those translations in the corresponding sites in the network
order: 0
---

This GraphQL query is an integration for the <a href="https://multilingualpress.org/" target="_blank">MultilingualPress</a> plugin, to translate tags.

The query must be executed on the master site (with source content), which must have the “All Extensions” bundle plugin installed. All other sites in the network can have the free Gato GraphQL plugin.
...
The query takes an origin tag, translates it using the Google Translate API to the other languages defined as connections via MultilingualPress, and stores those translations in the corresponding sites in the network. All translation tags must already exist.
...
By default it doesn't translate the tag slug. To enable, pass variable `updateSlug` with `true`.
...
# to translate tags, where every site in a
...
#
# It takes an origin tag on the master site, it translates it using the
...
# sites in the network. All translation tags in the network sites must
# already exist.
#
  ...
  transformedMetaName: _echo(value: $name)
    @underEachArrayItem(
      ...
      @applyField(
        ...
        passOnwardsAs: "transformedTagNameItem"
      )
      @applyField(
        name: "_arrayItem",
        arguments: {
          array: $transformedTagNameItem
          position: $itemNumber
        }
        passOnwardsAs: "transformedTagNameAsArray"
      )
      @applyField(
        name: "_arrayItem",
        arguments: {
          array: $transformedTagNameAsArray
          position: 0
        }
        setResultInResponse: true
      )
  ...
  transformedMetaDescription: _echo(value: $description)
    @underEachArrayItem(
      ...
      @applyField(
        ...
        passOnwardsAs: "transformedTagExcerptItem"
      )
      @applyField(
        name: "_arrayItem",
        arguments: {
          array: $transformedTagExcerptItem
          position: $itemNumber
        }
        passOnwardsAs: "transformedTagExcerptAsArray"
      )
      @applyField(
        name: "_arrayItem",
        arguments: {
          array: $transformedTagExcerptAsArray
          position: 0
        }
        setResultInResponse: true
      )
...
query ExportMutationInputs(
  ...
  @include(if: $executeTranslation)
{
  updateTagMutationInputDataItems: _echo(value: $networkSiteTranslationConnectionDataItems)
    @underEachArrayItem(
      passIndexOnwardsAs: "itemNumber"
    ...
    @export(
      as: "updateTagMutationInputDataItems"
    )
}
...
mutation UpdatePagesWithTranslationOnExternalSite
  ...
  @include(if: $executeTranslation)
{
  updateExternalSitePageHTTPRequests: _echo(value: $updateTagMutationInputDataItems)
    @underEachArrayItem(
...
mutation UpdateTagFromMasterSite($input: JSONObject!) {
