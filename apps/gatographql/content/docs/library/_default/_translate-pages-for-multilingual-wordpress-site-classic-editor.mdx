---
title: Translate pages for multilingual WordPress site (Classic editor)
description: Translate classic editor pages to the language of a translation site, and create the corresponding pages on that site, in a Multisite WordPress network
order: 0
---

This GraphQL query helps create a Multilingual site where every site is the translation for some language. The content is based on the Classic editor, and it is most suitable with a [WordPress multisite network](https://developer.wordpress.org/advanced-administration/multisite/create-network/).
...
The query must be executed on the master site (with source content), which must have the “All Extensions” bundle installed. All other sites in the network can have the free Gato GraphQL plugin.
