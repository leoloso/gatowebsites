export default function PopularPosts() {
  return (
    <div className="mb-8">
      <h4 className="text-lg font-bold leading-snug tracking-tight mb-4">Popular on the blog</h4>
      <ul className="-my-2">
        <li className="flex py-2 border-b border-slate-200">
          <svg className="w-4 h-4 shrink-0 fill-current text-slate-400 mt-1 mr-3" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M15.686 5.695L10.291.3c-.4-.4-.999-.4-1.399 0s-.4.999 0 1.399l.6.599-6.794 3.697-1-1c-.4-.399-.999-.399-1.398 0-.4.4-.4 1 0 1.4l1.498 1.498 2.398 2.398L.6 13.988 2 15.387l3.696-3.697 3.997 3.996c.5.5 1.199.2 1.398 0 .4-.4.4-.999 0-1.398l-.999-1 3.697-6.694.6.6c.599.6 1.199.2 1.398 0 .3-.4.3-1.1-.1-1.499zM8.493 11.79L4.196 7.494l6.695-3.697 1.298 1.299-3.696 6.694z" />
          </svg>
          <article>
            <h3 className="font-medium mb-1">
              <a href="#0" className="text-slate-100 hover:text-purple-400 transition duration-150 ease-in-out">
                Create and Deploy a blog with Simple
              </a>
            </h3>
            <div className="text-sm text-slate-500">
              <span className="text-slate-400">By </span>
              <span className="font-medium">Knut Mayer</span>
            </div>
          </article>
        </li>
        <li className="flex py-2 border-b border-slate-200">
          <svg className="w-4 h-4 shrink-0 fill-current text-slate-400 mt-1 mr-3" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M15.686 5.695L10.291.3c-.4-.4-.999-.4-1.399 0s-.4.999 0 1.399l.6.599-6.794 3.697-1-1c-.4-.399-.999-.399-1.398 0-.4.4-.4 1 0 1.4l1.498 1.498 2.398 2.398L.6 13.988 2 15.387l3.696-3.697 3.997 3.996c.5.5 1.199.2 1.398 0 .4-.4.4-.999 0-1.398l-.999-1 3.697-6.694.6.6c.599.6 1.199.2 1.398 0 .3-.4.3-1.1-.1-1.499zM8.493 11.79L4.196 7.494l6.695-3.697 1.298 1.299-3.696 6.694z" />
          </svg>
          <article>
            <h3 className="font-medium mb-1">
              <a href="#0" className="text-slate-100 hover:text-purple-400 transition duration-150 ease-in-out">
                Why we think Simple is good for developers
              </a>
            </h3>
            <div className="text-sm text-slate-500">
              <span className="text-slate-400">By </span>
              <span className="font-medium">Micheal Osman</span>
            </div>
          </article>
        </li>
        <li className="flex py-2 border-b border-slate-200">
          <svg className="w-4 h-4 shrink-0 fill-current text-slate-400 mt-1 mr-3" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M15.686 5.695L10.291.3c-.4-.4-.999-.4-1.399 0s-.4.999 0 1.399l.6.599-6.794 3.697-1-1c-.4-.399-.999-.399-1.398 0-.4.4-.4 1 0 1.4l1.498 1.498 2.398 2.398L.6 13.988 2 15.387l3.696-3.697 3.997 3.996c.5.5 1.199.2 1.398 0 .4-.4.4-.999 0-1.398l-.999-1 3.697-6.694.6.6c.599.6 1.199.2 1.398 0 .3-.4.3-1.1-.1-1.499zM8.493 11.79L4.196 7.494l6.695-3.697 1.298 1.299-3.696 6.694z" />
          </svg>
          <article>
            <h3 className="font-medium mb-1">
              <a href="#0" className="text-slate-100 hover:text-purple-400 transition duration-150 ease-in-out">
                Getting started with Vue.js and Stripe
              </a>
            </h3>
            <div className="text-sm text-slate-500">
              <span className="text-slate-400">By </span>
              <span className="font-medium">Justin Park</span>
            </div>
          </article>
        </li>
        <li className="flex py-2 border-b border-slate-200">
          <svg className="w-4 h-4 shrink-0 fill-current text-slate-400 mt-1 mr-3" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M15.686 5.695L10.291.3c-.4-.4-.999-.4-1.399 0s-.4.999 0 1.399l.6.599-6.794 3.697-1-1c-.4-.399-.999-.399-1.398 0-.4.4-.4 1 0 1.4l1.498 1.498 2.398 2.398L.6 13.988 2 15.387l3.696-3.697 3.997 3.996c.5.5 1.199.2 1.398 0 .4-.4.4-.999 0-1.398l-.999-1 3.697-6.694.6.6c.599.6 1.199.2 1.398 0 .3-.4.3-1.1-.1-1.499zM8.493 11.79L4.196 7.494l6.695-3.697 1.298 1.299-3.696 6.694z" />
          </svg>
          <article>
            <h3 className="font-medium mb-1">
              <a href="#0" className="text-slate-100 hover:text-purple-400 transition duration-150 ease-in-out">
                Making New projects with React & Simple
              </a>
            </h3>
            <div className="text-sm text-slate-500">
              <span className="text-slate-400">By </span>
              <span className="font-medium">Knut Mayer</span>
            </div>
          </article>
        </li>
        <li className="flex py-2">
          <svg className="w-4 h-4 shrink-0 fill-current text-slate-400 mt-1 mr-3" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg">
            <path d="M15.686 5.695L10.291.3c-.4-.4-.999-.4-1.399 0s-.4.999 0 1.399l.6.599-6.794 3.697-1-1c-.4-.399-.999-.399-1.398 0-.4.4-.4 1 0 1.4l1.498 1.498 2.398 2.398L.6 13.988 2 15.387l3.696-3.697 3.997 3.996c.5.5 1.199.2 1.398 0 .4-.4.4-.999 0-1.398l-.999-1 3.697-6.694.6.6c.599.6 1.199.2 1.398 0 .3-.4.3-1.1-.1-1.499zM8.493 11.79L4.196 7.494l6.695-3.697 1.298 1.299-3.696 6.694z" />
          </svg>
          <article>
            <h3 className="font-medium mb-1">
              <a href="#0" className="text-slate-100 hover:text-purple-400 transition duration-150 ease-in-out">
                How to add custom icons to your Simple project
              </a>
            </h3>
            <div className="text-sm text-slate-500">
              <span className="text-slate-400">By </span>
              <span className="font-medium">Lisa Allison</span>
            </div>
          </article>
        </li>
      </ul>
    </div>
  )
}
