...
That is something that could eventually be worked-upon (for instance, by releasing a **Gato GraphQL Lite** plugin, which simply installs a GraphQL server for internal consumption only, and not visible through the UI).

However, the most important question is: Would the new site become less secure, as it now offers a public GraphQL endpoint that any visitor can access? Could it unintentionally expose private data?
