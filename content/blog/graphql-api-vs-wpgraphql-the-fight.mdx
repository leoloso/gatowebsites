---
title: "🥊 Gato GraphQL vs WPGraphQL: the fight!"
description: 2 heavyweights bout to become the 'GraphQL in WordPress' world champion
...
🥁 Give 🥁 the 🥁 welcome 🥁 tooooo 🥁 ...... <a href="https://www.wpgraphql.com" rel="nofollow">WPGraphQL</a>!
...
🥁 Give 🥁 the 🥁 welcome 🥁 tooooo 🥁 ...... [Gato GraphQL](/)!

![The challenger, Gato GraphQL](/images/box/graphql-api-boxer.jpg "The challenger, Gato GraphQL")
...
- [Popularity](#popularity)
  - [Round 1: **Who is using it, and how complete is it**](#round-1-who-is-using-it-and-how-complete-is-it)
  - [Round 2: **Availability of extensions**](#round-2-availability-of-extensions)
  - [Round 3: **Target audience**](#round-3-target-audience)
- [Code style and standards](#code-style-and-standards)
  - [Round 4: **PHP requirements**](#round-4-php-requirements)
  - [Round 5: **Coding practices**](#round-5-coding-practices)
  - [Round 6: **Extending the schema**](#round-6-extending-the-schema)
- [Intermission](#intermission)
- [Pressing matters](#pressing-matters)
  - [Round 7: **Persisted queries**](#round-7-persisted-queries)
  - [Round 8: **Caching**](#round-8-caching)
  - [Round 9: **Integration with Gutenberg**](#round-9-integration-with-gutenberg)
- [Widening the scope](#widening-the-scope)
  - [Round 10: **Support for custom directives**](#round-10-support-for-custom-directives)
  - [Round 11: **Support for REST**](#round-11-support-for-rest)
  - [Round 12: **Support for novel features**](#round-12-support-for-novel-features)
- **[Verdict!](#verdict)**
...
Development for Gato GraphQL started in earnest around 1.5 years ago (as part of a wider project), and it reached a ["good enough" status 6 months ago](https://leoloso.com/posts/introducing-the-graphql-api-for-wordpress/), receiving [150 stars on the repo](https://github.com/GatoGraphQL/GatoGraphQL) since then. The plugin is currently [on version 0.7](https://github.com/GatoGraphQL/GatoGraphQL/releases/download/0.7.9/graphql-api.zip), and it's still several months away from reaching 1.0 (for instance, it doesn't have categories on the schema yet).

Last month I launched this current site gatographql.com, and since then I have been promoting the plugin via [the blog](/blog) (like the article you are reading now), and also published an [intro article on CSS-Tricks](https://css-tricks.com/rendering-the-wordpress-philosophy-in-graphql/). These attempts have brought in several hundred people to the site, and over 100 visitors have downloaded the plugin.
...
![It's a hit! WPGraphQL's punch reaches Gato GraphQL](/images/box/wpgraphql-hits.jpg "It's a hit! WPGraphQL's punch reaches Gato GraphQL")
...
![Modules page in Gato GraphQL](/assets/guides/downstream/modules.png "Modules page in Gato GraphQL")
...
Gato GraphQL, instead, [follows the WordPress philosophy that anyone should be able to use it](https://css-tricks.com/rendering-the-wordpress-philosophy-in-graphql/#reconciling-the-wordpress-philosophy-within-the-graphql-service), including non-techies. To achieve this goal, it enables to create and manage a GraphQL query via the WordPress editor, so that making the WordPress site's data accessible via an API becomes as easy as creating a blog post.

In addition, Gato GraphQL puts more emphasis on offering clients to interact with the GraphQL service in a visual manner. While both plugins provide the [GraphiQL client](https://github.com/graphql/graphiql), to execute the query, only Gato GraphQL also provides the [Voyager client](https://github.com/APIs-guru/graphql-voyager), to interactively explore the schema:
...
![Gato GraphQL delivers a good left blow!](/images/box/graphql-api-hits.jpg "Gato GraphQL delivers a good left blow!")
...
However, there is a difference: Gato GraphQL is actually [coded using PHP 7.4](https://github.com/GatoGraphQL/GatoGraphQL#php-versions), and is then [transpiled to PHP 7.1 for production](https://blog.logrocket.com/transpiling-php-code-from-8-0-to-7-x-via-rector/).
...
![The Gato GraphQL is leaving his mark!](/images/box/graphql-api-hits-again.jpg "The Gato GraphQL is leaving his mark!")
...
Let's switch over to the [`GatoGraphQL/GatoGraphQL` repo](https://github.com/GatoGraphQL/GatoGraphQL), which is the monorepo containing Gato GraphQL (among other projects).
...
✅ The underlying GraphQL server [is CMS-agnostic](/architecture/cms-agnosticism). Gato GraphQL implements the contracts for WordPress, and adds a bit of custom logic (for instance, to provide the clients).
...
✅ Likewise, [the GraphQL schema is not a 1:1 replica of the WordPress data model](/blog/rejuvenating-wordpress-through-graphql), bypassing the technical debt accumulated by WordPress at the data layer, and providing a clean interface.

✅ GraphQL's N+1 problem cannot happen, [by architectural design](/architecture/suppressing-n-plus-one-problem), and without troubling the developer at all.
...
✅ When generating the code for distribution, it [is scoped with PHP-Scoper](/blog/graphql-api-for-wp-is-now-scoped-thanks-to-php-scoper), and the source code, which contains PHP 7.4 code, which [is transpiled to PHP 7.1](https://blog.logrocket.com/coding-in-php-7-4-and-deploying-to-7-1-via-rector-and-github-actions/).
...
✅ [Fields are subscribed to types](/architecture/subscribing-fields-to-types), making the GraphQL schema easy to extend.
...
(In the meantime, I should display an ad from [my sponsors](https://github.com/sponsors/leoloso/). Unfortunately, I don't have any yet. If you'd like your company to fund development of Gato GraphQL, and get exposure in prime media like this event, [send me a message](/contact).)
...
Which brings us to Gato GraphQL. It not only [offers persisted queries](/extensions/persisted-queries), but has strived to make it a central part of the offering:

✅ The plugin comes with the single endpoint [disabled by default](/features/security), and users are encouraged to expose data via persisted queries only.
...
Check out the experience of [creating](/guides/use/creating-a-persisted-query) and executing a persisted query in my plugin:
...
Gato GraphQL has been smarter about this topic. Since it provides persisted queries, meaning that queries are executed on their own endpoint, it allows to [cache these endpoint URLs via HTTP caching](/features/http-caching).

The HTTP caching header has the `max-age` value automatically calculated from all the `max-age` values for all fields in the query, and this information is [configured using the WordPress editor](/guides/use/adding-http-caching), on a field-by-field basis.
...
WPGraphQL does not ship with a single [directive](https://graphql.org/learn/queries/#directives). I'm not saying it doesn't support them (it's engine `webonyx/graphql-php` does), but that it doesn't offer an implementation of any custom directive.
...
Gato GraphQL treats directives with reverence. They are [executed only once](/architecture/directive-design) with the data from all entities, for all the fields they are applied to (which explains why the `@strTranslate` directive can fetch results from the Google Translate API so quickly), and the GraphQL engine itself is based on a [directive pipeline](/architecture/directive-pipeline).
...
Concerning Gato GraphQL. I must admit I've been lucky, because work on its underlying engine (the server-side component model called [PoP](https://github.com/GatoGraphQL/GatoGraphQL)) started circa 2013, that is several years before I knew of something called GraphQL, and this project evolved with some ideas of its own (which I documented in [this vintage article of mine](https://www.smashingmagazine.com/2019/01/introducing-component-based-api/)).
...
✅ [Multiple query execution](/features/multiple-query-execution)<br/>
✅ [Schema namespacing](/features/schema-namespacing)<br/>
✅ [Nested mutations](/features/nested-mutations)<br/>
✅ [Composable directives](/features/composable-directives)<br/>
✅ [Proactive feedback](/features/proactive-feedback)<br/>
✅ [Field and directive-based versioning](/features/field-and-directive-based-versioning)
...
👑 The Gato GraphQL has a better architecture, and it could potentially better serve WordPress in the long run.
...
Or will you give Gato GraphQL the opportunity it is claiming for, [download the plugin](/download), and give it a try?
...
Update 01/05/2024: <strong>Learn more in the [Gato GraphQL vs WPGraphQL](/comparisons/gatographql-vs-wpgraphql) comparison.</strong>
