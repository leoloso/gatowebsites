...
To replicate REST endpoint `/wp-json/wp/v2/posts/{id}/` that retrieves data for the post with given ID, we can provide the post ID under URL param `postId`.
...
The Next.js WordPress starter [`leoloso/next-wordpress-starter`](https://github.com/leoloso/next-wordpress-starter) contains several examples of adapting queries between the two servers, in such a way that only the GraphQL query is adapted, while the JS logic remains the same. (This is accomplished by using GraphQL aliases, printing the response of the fields using with the same name as before.)

For instance, [this WPGraphQL query](https://github.com/leoloso/next-wordpress-starter/blob/main/src/data/providers/wpgraphql/posts.js#L3):
...
...can be adapted [like this for Gato GraphQL](https://github.com/leoloso/next-wordpress-starter/blob/main/src/data/providers/gatographql/posts.js#L3):
