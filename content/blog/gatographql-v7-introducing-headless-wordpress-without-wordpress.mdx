...
**Modules are directly mapped to underlying PHP packages** This means that, when running Gato GraphQL as a standalone, we can selectively load only those modules/packages that we need.

Then, if you application exposes data for posts, categories and tags only, when migrating away from WordPress only the `posts-wp`, `categories-wp`, and `tags-wp` packages need to be reimplemented, and nothing else.
...
## Demonstration: Using Gato GraphQL as a standalone PHP app
