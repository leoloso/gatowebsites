...
This video demonstrates using GraphQL to interact with GitHub's API:
...
In the video, the GraphQL query executes an HTTP request to fetch the latest Gato GraphQL plugins generated in GitHub Actions during development, uploaded as artifacts when merging a pull request.

The GraphQL response can then be injected with WP-CLI, as to have the DEV plugins automatically installed in my local DEV webserver, for testing them.

In this use case, as there is no WordPress data accessed, the GraphQL server can run as a standalone PHP app. (It could even be invoked within GitHub Actions!)

I'll explain in more detail how that GraphQL query works later on in this post.
