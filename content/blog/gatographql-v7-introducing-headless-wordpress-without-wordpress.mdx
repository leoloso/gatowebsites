...
Ever since the Matt Mullenweg vs WPEngine debacle, I've noticed more and more people (in Reddit and elsewhere) asking for alternatives to WordPress, not necessarily to switch away from WordPress (at least not immediately), but to understand what options they have, and how painful would a potential migration be. They want to hedge their bets.

For this folks who are working with Headless WordPress, Gato GraphQL has a cool new feature: **Headless WordPress without WordPress**.

Basically, Gato GraphQL has been built using standalone PHP components, managed via Composer. **All the PHP components making up the GraphQL server do not depend on WordPress!**
...
However, Gato GraphQL has these resolvers implemented via 2 packages: a non-WordPress one, containing all generic code, and a WordPress-specific one, containing the actual invocations to WordPress methods that satisfy that resolver (such as `get_posts` to fetch posts).
