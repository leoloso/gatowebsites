...
This new feature enhances the [Internal GraphQL Server](/extensions/) extension: It now accepts a `$schemaConfigurationIDOrSlug` parameter on methods `executeQuery` and `executeQueryInFile` from the `GraphQLServer` class, and already extracts the schema configuration used by the persisted query on `executePersistedQuery`:
...
The **Automation** extension now provides an "automator" user interface (directly via the WordPress editor) called [Automation Configurator](/extensions/automation/#heading-automation-configurator). The automation trigger is any WordPress action hook, and the action is the execution of a GraphQL persisted query.
