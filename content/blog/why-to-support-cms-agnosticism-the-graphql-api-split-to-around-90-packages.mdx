...
Last week I published article [💁🏻‍♀️ Why the GraphQL API needs a Monorepo, and How it's optimized](/blog/extending-the-monorepo-builder/), explaining how and why the `GatoGraphQL/GatoGraphQL` monorepo, which hosts the code for the GraphQL API for WordPress, can manage the plugin's codebase efficiently.
