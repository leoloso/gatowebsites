...
When using Gato GraphQL with WordPress `v6.6`, blocks in the plugin stopped working, and had to be re-compiled targetting WordPress `v6.0`+.

That's why Gato GraphQL `v3.0` requires at least WordPress `v6.0`.
...
Schema configuration block "Payload Types for Mutations" has been added a new option value (see below), hence its inner data structure has changed.
