---
title: "👨🏻‍🔧 Introducing Headless WordPress without WordPress"
seoTitle: "👨🏻‍🔧 Introducing Headless WordPress without WordPress in Gato GraphQL"
...
This gives us the following benefit:

**The GraphQL server can run as a standalone PHP application**, and you can include it within PHP application, WordPress or else.
...
This video demonstrates such a use case: Interacting with GitHub's API, to download/install artifacts during development:
...
In the video, the GraphQL query executes an HTTP request to fetch the latest Gato GraphQL plugins generated in GitHub Actions during development, which are uploaded as artifacts when merging a pull request.

The locations of the artifacts from the GraphQL response are then injected into WP-CLI, as to have the plugins automatically installed in my local DEV webserver, to run tests. (I'll explain how this process works later on in this post.)

In this use case, as there is no WordPress data accessed at all, the GraphQL server can already run as a standalone PHP app. (If needed, it could even be invoked within GitHub Actions!)
...
If you application exposes data for posts, categories and tags only, only the `posts-wp`, `categories-wp`, and `tags-wp` packages (along with their dependencies) need to be loaded.

Then, when migrating away from WordPress, only those 3 WordPress-specific packages would need to be reimplemented for the new framework/CMS, and nothing else.

This gives us another benefit:

**You can use headless WordPress today**, knowing that down the road you can **migrate your application to another framework or CMS with minimal effort**.
...
Fortunately, it is possible to swap them with Gato GraphQL. By doing that, you gain the ability to migrate your headless WordPress app away from WordPress.

These 2 steps would then be needed:

1. Transition from WP REST API or WPGraphQL to Gato GraphQL
2. Reimplement the WordPress-specific packages to load the needed data (as explained earlier on)
...
The Next.js WordPress starter [`leoloso/next-wordpress-starter`](https://github.com/leoloso/next-wordpress-starter) runs with either WPGraphQL or Gato GraphQL, using the same JS logic for the application, only the GraphQL queries are different.
...
## Using Gato GraphQL as a standalone PHP app
...
The PHP logic initializes the GraphQL server, and executes the query against it.
