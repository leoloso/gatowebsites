...
In this video, Polylang is configured with 3 languages: English (default), Spanish and French. We first translate a post to all languages by running a Persisted Query, and then publish an Automation Rule to translate the post automatically:
...
Gato GraphQL provides directive [`@strTranslate`](/extensions-reference/google-translate), which connects to the **Google Translate API** to translate the value in the corresponding field.

Thanks to the ability to [retrieve block data](/guides/interact/working-with-gutenberg-blocks) and extract all properties from within the block, we are able to translate block properties, insert them back into each block, and store the post back to the DB.
...
Gato GraphQL already ships with this persisted query, under title **[PRO] Translate post (Gutenberg)**
