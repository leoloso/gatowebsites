...
Thanks to the ability to [retrieve block data](/guides/interact/working-with-gutenberg-blocks) and extract all properties from within the block, we are able to translate these properties, insert them back into each block, and store the post back to the DB.
