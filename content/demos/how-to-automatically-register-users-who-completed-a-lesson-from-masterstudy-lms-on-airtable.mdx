...
The automation will be hooked on Masterstudy's action `stm_lms_lesson_passed`, which provides the following data:

```php
do_action( 'stm_lms_lesson_passed', $user_id, $lesson_id, $course_id );
```

The persisted query to execute will then connect to the [AirTable API](https://airtable.com/developers/web/api/introduction), create the records with the provided data.
