...
- The **provisional site** (`translation-es.instawp.xyz`), for the translator to fix the translation
- The **translated site** (`content-es.instawp.xyz`), containing the content in Spanish

Publish the blog post in your WordPress site and, depending on using the Gutenberg editor or the Classic editor, translate it to the desired language using Gato GraphQL + Google Translate, as explained in demos:

- [How to translate posts including blocks with the Google Translate API](how-to-translate-posts-including-blocks-with-the-google-translate-api)
- [...](...)

The **origin site** will now contain the Google-Translated post as a duplicate from the original, with `draft` status.
...
This persisted query will synchronize the post from the **origin site** to the **provisional site**.

Executing it, providing the JSON dictionary with the needed GraphQL variables:
...
You can now ask the translator to fix the translation, directly within the WordPress editor.
