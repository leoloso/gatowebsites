...
The **origin site** needs to have **Gato GraphQL PRO** installed, while the other two sites only need the free **Gato GraphQL plugin**.
...
When the translation is finished, we must synchronize it back to the **origin site**.

Create a persisted query containing the following GraphQL query, and give it title **Import post from another WordPress site**:
