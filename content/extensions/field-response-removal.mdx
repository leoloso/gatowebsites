...
In the query below, we generate the URL to send an HTTP request to, using fields to calculate intermediate values, which are not needed in the response, and we can `@remove` them:
