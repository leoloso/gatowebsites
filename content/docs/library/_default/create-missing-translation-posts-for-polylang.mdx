---
title: Create missing translation posts for Polylang
description: Given a post, duplicate it into all the other languages defined in Polylang for which there is no translation post yet
order: 0
---

This GraphQL query is an integration for the <a href="https://wordpress.org/plugins/polylang/" target="_blank">Polylang</a> plugin (and also **Polylang PRO**), to create the missing translation posts.
  ...
  createTranslationPostInputs: _echo(value: $missingTranslationLanguages)
    @underEachArrayItem(
      ...
      @applyField(
        ...
        arguments: {
          string: "%s (%s)"
          values: [$originPostRawTitle, $language]
        }
