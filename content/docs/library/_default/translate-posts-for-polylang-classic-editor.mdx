...
This GraphQL query is an integration for the <a href="https://wordpress.org/plugins/polylang/" target="_blank">Polylang</a> plugin (and also **Polylang PRO**), to translate posts based on the Classic editor.
  ...
  @include(if: $hasTranslationPosts)
{
  ...
  translationPosts: posts(filter: { ids: $__translationPostIds, status: $statusToUpdate } ) {
    id
