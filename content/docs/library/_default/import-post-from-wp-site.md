...
metaDesc: Create a new post using the data exposed by the GraphQL API on some other WordPress site
...
The Gato GraphQL plugin (free version) must be installed on the upstream website.
...
Variable `$upstreamServerGraphQLEndpointURL` is the Gato GraphQL endpoint on the upstream WordPress site, and `$postSlug` is the slug of the post to import.
