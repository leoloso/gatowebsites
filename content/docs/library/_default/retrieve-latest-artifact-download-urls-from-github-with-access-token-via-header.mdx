...
This query is similar to [Retrieve the latest artifact download URLs from GitHub](/library/retrieve-latest-artifact-download-urls-from-github), with the difference that it receives the GitHub credentials via the header `X-Github-Access-Token` in the request.
