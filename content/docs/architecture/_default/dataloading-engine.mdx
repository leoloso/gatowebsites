...
Gato GraphQL [uses server-side components](./using-components-instead-of-graphs) to represent the data model (not graphs or trees). Let's see how it executes the data-loading process to resolve the GraphQL query.
