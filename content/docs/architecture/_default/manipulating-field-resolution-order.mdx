...
That's why field `posts(filter:\{ search: $_authorName \})` is placed inside a `self` field in the query above, and running the query produces the expected response:
