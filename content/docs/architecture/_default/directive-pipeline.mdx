...
description: Directives are placed in a pipeline, and executed in order.
order: 800
---

Directives are placed in a pipeline, and executed in order. Their initial design is simple, like this:

![Directive pipeline](/assets/architecture/directive-pipeline.png)
...
![Directive pipeline with @validate and @resolveValueAndMerge](/assets/architecture/field-directive-pipeline.png)
...
- The `"after-resolve"` slot: after the field resolution
- The `"end"` slot: at the very end

The directive pipeline now looks like this (considering just 3 stages, to simplify):

![Directive pipeline with slots](/assets/architecture/slot-directive-pipeline.png)
...
![@skip directive in pipeline](/assets/architecture/skip-directive-pipeline.png)
...
![Receiving multiple fields as input in the directive pipeline](/assets/architecture/multiple-fields-directive-pipeline.png)
...
![The query requires 2 directive pipelines to be resolved](/assets/architecture/two-distinct-field-directive-pipeline.png)
...
![The query requires 3 directive pipelines to be resolved](/assets/architecture/three-distinct-field-directive-pipeline.png)
...
![Single directive pipeline to resolve all fields](/assets/architecture/1st-single-multiple-fields-slot-directive-pipeline.png)
...
![Single directive pipeline to resolve all fields](/assets/architecture/2nd-single-multiple-fields-slot-directive-pipeline.png)
...
![Pipeline with @getCache and @cache directives](/assets/architecture/cache-single-multiple-fields-slot-directive-pipeline.png)
...
![Passing the IDs per field to each directive](/assets/architecture/id-single-multiple-fields-slot-directive-pipeline.png)

Applied to the previous example, when executing the first query translating 2 records, the pipeline looks like this:

![Passing the IDs per field to each directive for the 1st query](/assets/architecture/1st-id-single-multiple-fields-slot-directive-pipeline.png)
...
![Passing the IDs per field to each directive for the 2nd query](/assets/architecture/2nd-id-single-multiple-fields-slot-directive-pipeline.png)
...
![Final design of the directive pipeline](/assets/architecture/multiple-fields-slot-directive-pipeline.png)
