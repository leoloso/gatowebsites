...
In guide [Complementing WP-CLI](/guides/code/complementing-wp-cli/), we only retrieved (and injected into WP-CLI) a single user ID. Now, let's retrieve multiple user IDs while executing a single GraphQL query.
...
- We use **[Multiple Query Execution](/extensions-reference/multiple-query-execution/)** to export a list of the user IDs, under dynamic variable `$userIDs`
