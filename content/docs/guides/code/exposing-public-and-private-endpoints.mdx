...
To tighten up the security of the API, we can use the [**Access Control**](/extensions/access-control/) extension to [grant access to the endpoint](/guides/use/defining-access-control/) only when:

- The user is logged-in (or not)
- The user has some role
- The user has some capability
- The visitor comes from an allowed IP (via the [**Access Control: Visitor IP**](/extensions/access-control-visitor-ip/) extension)
...
This is accomplished through the [**Cache Control**](/extensions/cache-control/) extension, which automatically calculates and outputs the response's `max-age` value based on the fields and directives present in the query.
