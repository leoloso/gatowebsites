...
Endpoints are configured via a [Schema Configuration](/guides/use/creating-a-schema-configuration), where we define:
...
If we have a configuration that we want to apply to all or most endpoints, we can define a [default Schema Configuration](/guides/config/defining-the-default-schema-configuration).

</Banner>

## When to use the single endpoint

The [single endpoint](/guides/config/enabling-and-configuring-the-single-endpoint) is always public, exposed by default under `/graphql`, and disabled by default (so the "Single Endpoint" module must be enabled).

<Banner type="info">

Gato GraphQL is managed via "modules", each of them offering some functionality or extension of the GraphQL schema, and which [can be enabled and disabled](/guides/config/browsing-enabling-and-disabling-modules) as needed.
...
[Custom endpoints](/guides/use/creating-a-custom-endpoint) are similar to the single endpoint, but we can have many of them, each exposed under its own URL `graphql/{custom-endpoint-slug}/`, with each of them having a different configuration.
...
To tighten up the security of the API, we can use the **[Access Control](/extensions-reference/access-control)** extension to [grant access to the endpoint](/guides/use/defining-access-control) only when:

- The user is logged-in (or not)
- The user has some role
- The user has some capability
- The visitor comes from an allowed IP (via the **[Access Control: Visitor IP](/extensions-reference/access-control-visitor-ip)** extension)
...
Gato GraphQL implements custom endpoints via Custom Post Types (CPTs). This allows us to [publish the custom endpoint as `private`](/features/public-private-and-password-protected-endpoints#private-endpoints) (and also as `password-protected`), thus making the custom endpoint accessible only to logged-in users who have the right to access that custom post, and nobody else.
...
[Persisted queries](/guides/use/creating-a-persisted-query) are endpoints, each having its own URL, but the GraphQL query is already defined on the server-side, hence the response is also predefined (it can be made dynamic by defining variables, to be satisfied by URL params).
...
We can benefit from this and make the API faster via standard [HTTP Caching](/guides/use/adding-http-caching), caching the GraphQL response on the client-side or intermediate stages between the client and the server (such as a CDN).

This is accomplished through the **[Cache Control](/extensions-reference/cache-control)** extension, which automatically calculates and outputs the response's `max-age` value based on the fields and directives present in the query.
