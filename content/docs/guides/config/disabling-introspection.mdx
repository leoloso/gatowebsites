---
title: Disabling introspection [PRO]
# isPRO: true
description: Disabling introspection for the single endpoint or custom endpoints via Access Control.
order: 3610
---

The introspection `__schema` field is exposed in the [Access Control Lists](/guides/use/defining-access-control):
...
This allows us to disable introspection for the [single endpoint](/guides/config/enabling-and-configuring-the-single-endpoint) or [custom endpoints](/guides/use/creating-a-custom-endpoint), following any of the already available rules, such as:
