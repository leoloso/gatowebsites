...
Similar to the [`blockEditor` internal endpoint](/guides/special-features/internal-endpoint-for-blocks), developers can also create their own pre-defined internal endpoints (to feed data to their application or blocks), as to apply a specific configuration:
