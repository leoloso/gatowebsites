---
title: Working with (Gutenberg) blocks
description: This is how to query (Gutenberg) block data from the GraphQL schema.
# image: /assets/GatoGraphQL-logo-suki.png
order: 200
---

This is how to retrieve Gutenberg block data.

The GraphQL schema has the following fields added to all `CustomPost` types (such as `Post` and `Page`):

- `blocks`
- `blockDataItems`
- `blockFlattenedDataItems`

These fields are not enables if the [Classic Editor](https://wordpress.org/plugins/classic-editor/) plugin is active.
...
- `contentSource` retrieves the block's (Gutenberg) HTML source code, including the comment delimiters that contain the attributes. However, this field does not retrieve the exact same data as how it is stored in the DB (see <a href="https://github.com/GatoGraphQL/GatoGraphQL/issues/2346" target="_blank">#2346</a>), so use this field with care.
...
![Configuring to directly retrieve `GeneralBlock` instead of `BlockUnion`](/assets/guides/upstream/settings-blocks-single-type.png "Configuring to directly retrieve `GeneralBlock` instead of `BlockUnion`")
