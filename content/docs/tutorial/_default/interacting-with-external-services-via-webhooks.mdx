---
title: "Lesson 18: Interacting with external services via webhooks"
description: Tutorial lesson on creating Persisted Queries that act as webhooks.
...
Because the request is sent via `POST`, we must extract the data from the body of the HTTP request (which is supported via the **[HTTP Request via Schema](/extensions-reference/http-request-via-schema)** extension).

<Banner type="info">

If the HTTP request is executed via `GET`, then the Persisted Query can directly [obtain the data items from the URL parameters](/guides/use/creating-a-persisted-query#heading-making-the-persisted-query-dynamic-via-url-params).
...
The **[HTTP Request via Schema](/extensions-reference/http-request-via-schema)** extension allows us to retrieve all of the current HTTP request data, via the following fields:
