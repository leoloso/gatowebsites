---
title: "Lesson 16: Interacting with external services via webhooks"
...
If the HTTP request is executed via `GET`, then the Persisted Query can directly [obtain the data items from the URL parameters](/guides/use/creating-a-persisted-query#making-the-persisted-query-dynamic-via-url-params).
