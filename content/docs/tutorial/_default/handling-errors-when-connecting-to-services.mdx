---
title: Lesson 2640 - Handling errors when connecting to services
...
This GraphQL query captures this data, and explicitly adds an error entry with the response's error code and message, by using field `_fail` (provided by the [**Response Error Trigger**](/extensions/response-error-trigger/) extension):
...
The [**Response Error Trigger**](/extensions/response-error-trigger/) extension provides two ways to add a custom entry under `errors`:
