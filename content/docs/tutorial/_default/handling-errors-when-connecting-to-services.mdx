---
title: "Lesson 22: Handling errors when connecting to services"
description: How to skip executing the rest of the GraphQL query after an error occurs.
order: 2640
...
This GraphQL query captures this data, and explicitly adds an error entry with the response's error code and message, by using field `_fail` (provided by the **[Response Error Trigger](/extensions-reference/response-error-trigger)** extension):
...
The **[Response Error Trigger](/extensions-reference/response-error-trigger)** extension provides two ways to add a custom entry under `errors`:
