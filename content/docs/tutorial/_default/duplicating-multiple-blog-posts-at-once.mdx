---
title: "Lesson 4: Duplicating multiple blog posts at once"
description: Tutorial lesson on duplicating multiple posts with a single GraphQL request.
order: 1150
...
For this GraphQL query to work, the [Schema Configuration](/guides/use/creating-a-schema-configuration) applied to the endpoint needs to have the following configuration:

- In block [Payload Types for Mutations](/guides/config/returning-a-payload-object-or-the-mutated-entity-for-mutations), select "Do not use Payload Types for Mutations (i.e. return the mutated entity)" (so that dynamic variable `$createdPostIDs` will contain the IDs of the created posts)
- In block [Mutation Scheme](/guides/schema/using-nested-mutations), select any of the two "Enable Nested Mutations" options (as to use field `_echo` inside a `mutation`)
...
Thanks to the **[Field to Input](/extensions-reference/field-to-input)** extension, we can create the input object already on the first operation, and export all the required post data under a single dynamic variable:
...
- **[Field Value Iteration and Manipulation](/extensions-reference/field-value-iteration-and-manipulation)** provides directive `@underEachArrayItem` to iterate over all the items in `$postInput`
- **[Field on Field](/extensions-reference/field-on-field)** provides directive `@applyField`, to apply the `createPost` mutation on each iterated-upon item from the array

<Banner type="info">

`@underEachArrayItem` is a [composable directive](/guides/schema/using-composable-directives) (or "meta directive", it is a directive which can contain nested directives) that iterates over an array of elements, and applies its nested directive on each of them.
