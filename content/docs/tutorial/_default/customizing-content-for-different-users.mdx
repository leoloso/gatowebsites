---
title: "Lesson 5: Customizing content for different users"
description: Tutorial lesson on retrieving a different response in a field depending on some piece of queried data, such as the roles of the logged-in user.
...
When **[Multiple Query Execution](/extensions-reference/multiple-query-execution)** is enabled, directives `@include` and `@skip` can also be applied to operations. This way, we can execute an operation or not depending on the value of some dynamic variable.
