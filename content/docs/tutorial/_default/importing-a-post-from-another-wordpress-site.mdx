---
title: Lesson 3300 - Importing a post from another WordPress site
...
When we need to pass the field's resolved value to a _directive_, we must instead use directive `@passOnwards` (which is similarly provided by the [**Field to Input**](/extensions/field-to-input/) extension).
