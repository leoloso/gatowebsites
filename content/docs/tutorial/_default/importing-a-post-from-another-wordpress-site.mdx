---
title: Importing a post from another WordPress site
metaDesc: Tutorial lesson on how we can keep WordPress sites in sync, by importing a post from some WordPress site into our local WordPress site.
...
(As we've seen on previous tutorial lessons) We use the **Field to Input** feature (with syntax `$__field`) to pass the field's resolved value to a contiguous _field_.
