---
title: "Lesson 29: Importing a post from another WordPress site"
description: Tutorial lesson on how we can keep WordPress sites in sync, by importing a post from some WordPress site into our local WordPress site.
order: 3300
...
When we need to pass the field's resolved value to a _directive_, we must instead use directive `@passOnwards` (which is similarly provided by the **[Field to Input](/extensions-reference/field-to-input)** extension).
