---
title: "Lesson 17: Retrieving data from an external API"
...
As the content-type of the response is `application/json`, we can transform the raw body content from `String` to `JSONObject` via field `_strDecodeJSONObject` (from the **[PHP Functions Via Schema](/extensions-reference/php-functions-via-schema)** extension):
...
This is because whenever `@export` is nested below `@underEachArrayItem` (or `@underEachJSONObjectProperty`), the [exported value will already be an array](/guides/schema/executing-multiple-queries-concurrently#exporting-values-when-iterating-an-array-or-json-object).
