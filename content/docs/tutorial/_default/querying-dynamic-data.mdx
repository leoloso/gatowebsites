---
title: Lesson 250 - Querying dynamic data
...
The [**PHP Functions Via Schema**](/extensions/php-functions-via-schema/) extension provides many of the most common PHP functions as global fields, including:
...
`$__timeNow` is a variable dynamically created by the [**Field to Input**](/extensions/field-to-input/) extension, which allows us to obtain the value of a field and [input it into another field](/guides/schema/using-field-to-input/) in that same operation.
...
Field `_env` is provided via the [**PHP Constants and Environment Variables via Schema**](/extensions/php-constants-and-environment-variables-via-schema/) extension.
