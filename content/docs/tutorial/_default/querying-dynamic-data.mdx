---
title: "Lesson 2: Querying dynamic data"
description: Gato GraphQL allows us to dynamically compute data, input it back into the query, and affect the response with granular control. Here is how.
order: 250
...
Gato GraphQL provides function fields under the concept of [Global fields](/features/global-fields): Fields that are accessible under all the types from the GraphQL schema. (Normal fields in GraphQL, in contrast, are accessible only under some specific type, such as `Post` or `User`).

By convention, global fields in Gato GraphQL start with `_` (and normal fields do not).

The **[PHP Functions Via Schema](/extensions-reference/php-functions-via-schema)** extension provides many of the most common PHP functions as global fields, including:
...
`$__timeNow` is a variable dynamically created by the **[Field to Input](/extensions-reference/field-to-input)** extension, which allows us to obtain the value of a field and [input it into another field](/guides/schema/using-field-to-input) in that same operation.
...
Field `_env` is provided via the **[PHP Constants and Environment Variables via Schema](/extensions-reference/php-constants-and-environment-variables-via-schema)** extension.

Via the applied Schema Configuration and plugin Settings, we can configure [which constants and environment variables can be queried](/guides/config/configuring-what-environment-variables-and-php-constants-can-be-queried).
