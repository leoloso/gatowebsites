---
title: "Lesson 2700: Creating an API gateway"
description: Tutorial lesson on creating an API gateway that retrieves the latest artifacts from the GitHub Actions API, and extracts their URL to be downloaded, avoiding the need for the client to be signed in to GitHub.
...
The GraphQL query below must be [stored as a Persisted Query](/guides/use/creating-a-persisted-query/) (for instance, using slug `retrieve-public-urls-for-github-actions-artifacts`).
