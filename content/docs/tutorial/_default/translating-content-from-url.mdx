...
We can combine the fields to execute HTTP requests (provided by the **[HTTP Client](/extensions/http-client/)** extension) with directive `@strTranslate` (provided by the [**Google Translate](/extensions-reference/google-translate/)** extension), to translate the content from any URL.
