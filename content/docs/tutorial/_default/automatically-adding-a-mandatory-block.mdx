---
title: "Lesson 17: Automatically adding a mandatory block"
description: "Whenever a new post is created, we can use the automation features to validate and modify the content of the post. The query in this tutorial lesson checks if a certain mandatory block is present in the post and, whenever missing, it adds it."
order: 2000
...
Then, for this GraphQL query to work, the [Schema Configuration](/guides/use/creating-a-schema-configuration) applied to the **Internal GraphQL Server** needs to have [Nested Mutations](/guides/schema/using-nested-mutations) enabled.

</Banner>

This PHP code hooks into WordPress action `draft_post` to execute the Persisted Query (via the **[Internal GraphQL Server](/extensions-reference/internal-graphql-server)** extension):
