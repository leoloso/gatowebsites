---
title: "Lesson 30: Distributing content from an upstream to multiple downstream sites"
description: Tutorial lesson on implementing a distributed server architecture using Gato GraphQL extensions on the upstream WordPress site only.
...
_(For the downstream sites only)_ For this GraphQL query to work, the [Schema Configuration](/guides/use/creating-a-schema-configuration) applied to the endpoint needs to have [Nested Mutations](/guides/schema/using-nested-mutations) enabled
...
This is possible because of the difference between function fields `_notNull` and `_notEmpty` (provided by the **[PHP Functions via Schema](/extensions-reference/php-functions-via-schema)** extension):
