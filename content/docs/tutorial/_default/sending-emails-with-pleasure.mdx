---
title: Lesson 1600 - Sending emails with pleasure
...
We send emails via mutation `_sendEmail` provided by the [**Email Sender**](/extensions/email-sender/) extension.
...
Field `_strConvertMarkdownToHTML` from the [**Helper Function Collection**](/extensions/helper-function-collection/) extension converts Markdown to HTML.
...
Using the function fields provided by the [**PHP Functions via Schema**](/extensions/php-functions-via-schema/), we can create a message template containing placeholders, and replace them with dynamic data:
...
Alternatively, if [Nested Mutations](/guides/schema/using-nested-mutations/) is enabled in the Schema Configuration, we can retrieve the admin email in the `mutation` operation already (and inject it into the mutation via [**Field to Input**](/extensions/field-to-input/)):
