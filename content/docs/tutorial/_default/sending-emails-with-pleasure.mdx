---
title: "Lesson 14: Sending emails with pleasure"
description: Tutorial lessons demonstrating several capabilities by Gato GraphQL to send emails.
...
We send emails via mutation `_sendEmail` provided by the **[Email Sender](/extensions-reference/email-sender/)** extension.
...
Field `_strConvertMarkdownToHTML` from the **[Helper Function Collection](/extensions-reference/helper-function-collection/)** extension converts Markdown to HTML.
...
Using the function fields provided by the **[PHP Functions via Schema](/extensions-reference/php-functions-via-schema/)**, we can create a message template containing placeholders, and replace them with dynamic data:
...
Alternatively, if [Nested Mutations](/guides/schema/using-nested-mutations/) is enabled in the Schema Configuration, we can retrieve the admin email in the `mutation` operation already (and inject it into the mutation via **[Field to Input](/extensions-reference/field-to-input/)**):
