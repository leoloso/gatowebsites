...
Gato GraphQL provides the ability to compose fields, so that a foundational set of field resolvers can cover an unlimited number of use cases.

With [Field to Input](/features/field-to-input), you can retrieve the value of a field, and provide it as input to another field. For instance, instead of creating field `Post.hasExcerpt`, we can apply field `_notEmpty` on `Post.excerpt` to produce the same value:
