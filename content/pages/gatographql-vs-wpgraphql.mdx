...
You will have access to [Multiple Query Execution](/features/multiple-query-execution), where a single GraphQL document can execute multiple operations, which share state with one another via the `@export` directive.
