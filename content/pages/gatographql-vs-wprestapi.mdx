...
With the <a href="https://developer.wordpress.org/rest-api/" rel="nofollow">WP REST API</a>, you expose data via REST endpoints. Each endpoint has its own URL, and its data is pre-defined (for the corresponding resources, such as posts, users, etc).
...
The difference between REST API endpoints and GraphQL persisted queries is that:

- REST API endpoints are created via PHP code, and they must be deployed (within a theme or plugin)
- Gato GraphQL persisted queries are created using the [GraphQL language](https://graphql.org), and they are published directly within the wp-admin, without the need to deploy any code.
