...
Gato GraphQL provides [Multiple Query Execution](/features/multiple-query-execution), where a single GraphQL operation can execute multiple operations.

Multiple Query Execution is an improvement over batch requests, as the operations can share state with one another, whereas in the batch request they cannot.
