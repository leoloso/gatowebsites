...
- It is not a translation plugin, but you can [translate any content, including the properties inside Gutenberg blocks](https://gatographql.com/library/translate-post-gutenberg).
...
- It is not a search and replace plugin, but you can [modify your posts in bulk](/library/regex-search-and-replace-multiple-strings-in-all-posts), with a single query.
- It is not webhook plugin or HTTP client, but you can both [send a request to any API](/features/http-client), and [receive incoming requests from any service](/library/register-a-newsletter-subscriber-from-instawp-to-mailchimp).

What you can use Gato GraphQL for, literally depends on your imagination.

In contrast, the WP REST API is just that, an API.
